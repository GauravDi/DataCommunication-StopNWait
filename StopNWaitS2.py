...
while (CP<=Pkts1):  
    i=randint(0,100)                                                             #Packet drop function                                 
    #print("PACKET DROP")
    if i>0:              
...
#try:
sock.settimeout(2) 
#s=randint(0,100)                                                                 #Initialize wait state timer
data, address=sock.recvfrom(1024)
RFIN=data.decode()
print()
print("FINAL PACKET"+' '+RFIN[1:7]+' '+"WITH SEQ NO."+' '+RFIN[0]+' '+"FROM RECEIVER IS RECEIVED")
ACK="FINACK"
sock.sendto(ACK,(Y1,Y2))
print()
print("ACK FOR RECEIVER FIN PACKET IS SENT")
sock.settimeout(2)
print()
